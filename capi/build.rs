...
fn main() {
    ...
    //   Check:
    //     - build.rs for C/Rust glue options
    //     - src/capi/iso20-encoder.h for C prototype inputs
    // -----------------------------------------------------------------------
    ";
    println!("cargo:rerun-if-changed=capi/iso20-messages/capi-iso20.h");
    let libcapi = bindgen::Builder::default()
        .header("capi/iso20-messages/capi-iso20.h")
        .raw_line(header)
        .parse_callbacks(Box::new(bindgen::CargoCallbacks::new()))
        .derive_debug(true)
        .layout_tests(false)
        .allowlist_item("iso20_.*")
        .allowlist_function("decode_iso20_.*")
        .allowlist_function("encode_iso20_.*")
        .allowlist_function("iso20_sign_.*")
        .blocklist_item("gnutls_.*")
        .blocklist_item("exi.*")
        .generate()
        .expect("Unable to generate _iso20-capi.rs");

    libcapi
        .write_to_file("capi/iso20-messages/_iso20-capi.rs")
        .expect("Couldn't write capi/iso20-messages/_iso20-capi.rs!");
}
